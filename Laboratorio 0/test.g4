...
QUOTES          :       '"';
APOSTROPHE      :       '\'';
CHAR_LITERAL    :       ('"' ( LETRA ) '"') | (APOSTROPHE ( LETRA ) APOSTROPHE);
PARENTESIS      :       '('
                |       ')'
                ;
...
clase   :   'class' IDENTIFICADOR '{' (contenidoClase) '};'
        |   'class' IDENTIFICADOR 'inherits' IDENTIFICADOR '{' (declaracionClase) '};'
        |   'class Main' ('inherits')? (librerias)* '{' (declaracionClase) '};'
        ;

contenidoClase  :    (declaracionVariable)* (metodo)*
                ;

metodo  :       IDENTIFICADOR '():' librerias '{' {declaracionOperacion} '};'
        ;
